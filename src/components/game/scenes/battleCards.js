...
export default class battleCardScene extends Phaser.Scene {
	...
	create() {
		...
		this.devlingStats = Object.keys(playerDevling).filter(
			(stat) => typeof playerDevling[stat] === 'number'
		);
		...
		for (const [key, value] of Object.entries(playerDevling)) {
			if (typeof value === 'number') {
				this.add.text(centerX / 2, playerStatDistance, `${key}: ${value}`, {
					fontSize: 32,
					font: '"Press Start 2P"',
					fill: this.placeForText ? 'green' : 'black',
				});
		...
		for (const [key, value] of Object.entries(enemyDevling.stats)) {
			if (typeof value === 'number') {
				this.add.text(
					...
					enemyStatDistance,
					`${key}: ${value}`,
					{ fontSize: 32, font: '"Press Start 2P"', color: 'black' }
				);
